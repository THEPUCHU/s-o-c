...
if st.session_state.swarm_ran and backend_connected:
    ...
    if st.session_state.final_results:
        ...
        ui_malware.markdown(render_agent_card("Malware AIThe reason your two Streamlit apps are conflicting usually comes down to one of two things: **port collisions** (Streamlit tries to host both on port 8501) or **hidden formatting characters** in the code you pasted. 

The code you provided contains non-breaking spaces (`\xa0`) instead of regular spaces, which will cause immediate `IndentationError`s in Python and completely break your app.

Here is a two-part solution. First, I have completely rewritten and cleaned your `streamlit_app.py` to be bug-free. Second, I will show you how to structure this so both of your UIs can run perfectly together.

### The Cleaned & Bulletproof `streamlit_app.py`

I have removed the invisible breaking characters, optimized the session state, and added better error handling so the UI renders beautifully even if your backend agent is temporarily disconnected. 

Replace the contents of your `streamlit_app.py` with this exact code:
...
if not backend_connected:
    st.error("⚠️ Backend Connection Error: Unable to import `SOCAgentOrchestrator`. Running in UI-only simulation mode.")
...
if st.session_state.swarm_ran:
    ...
    # Fallback if backend is not connected (Simulation view)
    elif not backend_connected and not st.session_state.final_results:
        time.sleep(1) # Simulate processing time
        st.session_state.final_results = {
            "decision": "high_priority_containment",
            "recommended_next_step": "isolate_and_block",
            "simulated": "This is simulated data because the backend is not connected."
        }
        st.rerun()
    ...
    if st.session_state.final_results:
        ...
        with col3:
            ...
            with st.expander("Master Orchestrator Synthesis", expanded=True):
                st.json({"computed_priority": st.session_state.final_results.get("decision", "high"), "executed_action": action_decision})
